...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='16'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='16'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='16'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Крокодил</h1>
            <img src="krokodil.jpg">
            <p class="lead">Крокоди́лы (лат. Crocodilia) — отряд вторично-водных позвоночных, которых обычно относят к сборной (парафилитической) группе «пресмыкающихся». В рамках кладистики крокодилы рассматриваются как единственная выжившая субклада более широкой клады круротарзы. Хотя словом «крокодил» правильнее обозначать представителя семейства настоящих крокодилов, в целом этот термин употребим для обозначения любых представителей отряда, к которому также относятся аллигаторы, гавиаловые и т. д.</p>
            <hr class="my-4">
            <p>Всего сейчас крокодилов: <?php
                $query = "select a_count from animal where a_name = 'krokodil';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'krokodil';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='15'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='15'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='15'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Чешуйчатые</h1>
            <img src="chesh.jpg">
            <p class="lead">ешу́йчатые (лат. Squamata) — один из четырёх современных отрядов пресмыкающихся, включающий змей, ящериц, а также менее известных амфисбен, или двуходок. Животные этого отряда широко распространены во всех частях света на материках и островах, отсутствуют в полярных и приполярных регионах.</p>
            <hr class="my-4">
            <p>Всего сейчас чешуйчатых: <?php
                $query = "select a_count from animal where a_name = 'chesh';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'chesh';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

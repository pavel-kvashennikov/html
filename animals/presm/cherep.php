...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='14'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='14'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='14'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
    ...
    <form method="POST">
        <div class="jumbotron">
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'cherep';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);
            ...
            <input type="submit" class="btn btn-primary btn-lg" name="add" value="Добавить черепашку" />
            <input type="submit" class="btn btn-primary btn-lg" name="del" value="Удалить черепашку" />
            <?php
            $query = "select islock from animal where a_name = 'cherep';";
            $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
            if($result)

...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='3'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='3'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='3'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Лосось</h1>
            <img src="losos.jpg">
            <p class="lead">Лососёвые (лат. Salmonidae) — семейство в отряде лососеобразных (Salmoniformes).<br>В составе семейства представлены как анадромные, так и пресноводные виды рыб. Наиболее известные представители — сёмга, горбуша, кета, нерка, кижуч, чавыча, кумжа, сиг, омуль, голец, хариус, таймень, ленок. Хорошо известные собирательные названия нескольких разных видов — лосось и форель.</p>
            <hr class="my-4">
            <p>Всего сейчас лососей: <?php
                $query = "select a_count from animal where a_name = 'losos';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'losos';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

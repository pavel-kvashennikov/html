...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='5'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='5'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='5'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Змееголовые</h1>
            <img src="zmei.jpg">
            <p class="lead">Змееголовые[1] (лат. Channidae) — семейство пресноводных лучепёрых рыб из отряда Anabantiformes[2]. Известно два современных рода и один вымерший. Встречаются в экваториальной Африке (Parachanna) и южной, юго-восточной и восточной Азии (Channa); некоторые виды рода Channa были интродуцированы в Северную Америку[3]. Вымерший род Eochanna, включающий всего один вид — Eochanna chorlakkiensis, описан из Пакистана[4]. Рыбы средних или больших размеров. Взрослые особи Channa panaw достигают не более 17,1 см в длину, тогда как Channa marulius в длину бывают до 183 см и массой до 30 килограмм[5].</p>
            <hr class="my-4">
            <p>Всего сейчас карпов: <?php
                $query = "select a_count from animal where a_name = 'zmei';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'zmei';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='2'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='2'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='2'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<form method="POST">
    <div class="jumbotron">
        <h1 class="display-4">Карп</h1>
        <img src="karp.jpg">
        <p class="lead">Большая рыба коричневого или желто-зеленого цвета. Хотя цвет карпа может меняться в зависимости от места обитания. Тело карпа удлиненное и покрытое крупной золотисто-бурой чешуей. Бывают карпы практически без чешуи.</p>
        <hr class="my-4">
        <p>Всего сейчас карпов: <?php
            $query = "select a_count from animal where a_name = 'karp';";
            $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
            if($result)
            {

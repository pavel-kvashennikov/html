...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='6'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='6'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='6'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Хищные</h1>
            <img src="hishn.jpg">
            <p class="lead">Хи́щные (лат. Carnivora — ‘плотоядные’) — отряд плацентарных млекопитающих (Mammalia), состоящий из подотрядов собакообразных (Caniformia) и кошкообразных (Feliformia). Традиционное подразделение хищных на две сильно отличающиеся друг от друга по образу жизни группы — сухопутные хищные (Fissipedia) и ластоногие (Pinnipedia) — в настоящее время считается таксономически некорректным, поскольку противоречит имеющимся данным о филогении отряда[1][2]. В состав отряда входят 16 современных семейств и несколько вымерших[3][4]; современные хищные насчитывают свыше 280 видов в 128 родах и распространены почти по всему миру[4][5]. Известно также около 300 ископаемых родов хищных[6].</p>
            <hr class="my-4">
            <p>Всего сейчас хищных: <?php
                $query = "select a_count from animal where a_name = 'hishn';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'hishn';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='7'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='7'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='7'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Приматы</h1>
            <img src="primat.jpg">
            <p class="lead">Прима́ты (лат. Primates, фр. Primat, от primas, букв. «первые») — один из наиболее прогрессивных отрядов плацентарных млекопитающих, включающий, в том числе, обезьян и человека. Отряд насчитывает более 400 видов[1].
            ...
            <hr class="my-4">
            <p>Всего сейчас приматов: <?php
                $query = "select a_count from animal where a_name = 'primat';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'primat';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='13'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='13'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='13'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Сова</h1>
            <img src="sova.jpg">
            <p class="lead">Совообра́зные (лат. Strigiformes) — отряд хищных птиц, включающий более 200 крупных и средней величины видов, в основном ночных птиц, распространённых во всех странах мира. В отряде два современных семейства: совиные, или настоящие совы, и сипуховые.
            ...
            <hr class="my-4">
            <p>Всего сейчас сов: <?php
                $query = "select a_count from animal where a_name = 'sova';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'sova';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);

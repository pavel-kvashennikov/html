...
if( isset( $_POST['add'] ) )
{
    $query = "UPDATE animal SET a_count = a_count+1 WHERE idanimal='10'";
...
if (isset( $_POST['open']))
{
    $query = "UPDATE animal SET islock='0' WHERE idanimal='10'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
if (isset( $_POST['close']))
{
    $query = "UPDATE animal SET islock='1' WHERE idanimal='10'";
    $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
}
?>
...
<div class="container">
    <form method="POST">
        <div class="jumbotron">
            <h1 class="display-4">Аист</h1>
            <img src="aist.jpg">
            <p class="lead">Для аистов характерны длинные обнаженные ноги, покрытые сетчатой кожей; клюв длинный, прямой, конический; передние пальцы соединены между собой широкой плавательной перепонкой, пальцы короткие с розоватыми когтями; на голове и шее местами голая кожа.</p>
            <hr class="my-4">
            <p>Всего сейчас аистов: <?php
                $query = "select a_count from animal where a_name = 'aist';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
            ...
            <p>Состояние клетки: <?php
                $query = "select islock from animal where a_name = 'aist';";
                $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
                if($result)
                {
                    $row = mysqli_fetch_row($result);
            ...
            <?php
            $query = "select islock from animal where a_name = 'karp';";
            $result = mysqli_query($link, $query) or die ("Ошибка " . mysqli_error($link));
            if($result)
            {
                $row = mysqli_fetch_row($result);
